...
void survivalRule(char life[][20]) // fix 4, removed unused x and y params
{
      int row, col;
      int neighbors = 0;
...
void birthRule(char life[][20]) // fix 3 removed unused params x and y
{
      int row, col;
      int neighbors = 0;
      for(row = 1; row<19; row++)
...
// fix 4 removed unused variable ans
      ...
      for (int i = 1; i<=51; i++) {
          birthRule(life); // fix 5 fixed function call to not use unused vars
          survivalRule(life); // fix 5 fixed function call to not use unused vars
          for(row = 0; row<20; row++)
          {
              for(col = 0; col<20; col++)
              {
